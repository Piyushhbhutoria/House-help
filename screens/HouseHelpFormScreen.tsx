...
const HouseHelpFormScreen: React.FC = () => {
  ...
  const { addHouseHelp, updateHouseHelp, deleteHouseHelp, houseHelps } = useHouseHelp();
  ...
  const isEditMode = !!houseHelpId;
  const currentHouseHelp = isEditMode ? houseHelps.find((hh) => hh.id === houseHelpId) : null;

  useEffect(() => {
    if (houseHelpId) {
      // Editing existing house help
      if (currentHouseHelp) {
        setName(currentHouseHelp.name);
        setMonthlySalary(currentHouseHelp.monthlySalary.toString());
        setShifts(currentHouseHelp.shifts.toString());
        setWorkingDays(currentHouseHelp.workingDays || [1, 2, 3, 4, 5]);
      }
    } else {
      // Adding new house help - use default working days from settings
  ...
  }, [houseHelpId, currentHouseHelp, settings.defaultWorkingDays]);
  ...
  const handleSave = async () => {
    ...
    try {
      const houseHelpData = {
        ...
        workingDays,
        overtimeRate: currentHouseHelp?.overtimeRate || 0,
        holidayRate: currentHouseHelp?.holidayRate || 0,
        advancePayment: currentHouseHelp?.advancePayment || 0,
        adjustments: currentHouseHelp?.adjustments || 0,
        multipleShifts: currentHouseHelp?.multipleShifts || false,
      };

      if (houseHelpId) {
        await updateHouseHelp(houseHelpId as string, houseHelpData);
        Alert.alert(
          '✅ Updated!',
      ...
      console.error('Error saving house help:', error);
      Alert.alert('Error', `Failed to ${isEditMode ? 'update' : 'add'} house help. Please try again.`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    const executeDelete = async () => {
      try {
        setIsLoading(true);
        await deleteHouseHelp(houseHelpId as string);
        Alert.alert(
          '✅ Deleted!',
        ...
        Alert.alert('Error', 'Failed to delete house help. Please try again.');
        setIsLoading(false);
      }
    };
    ...
    if (settings.confirmDeletions) {
      Alert.alert(
        '🗑️ Delete House Help',
        `Are you sure you want to delete ${name}? This action cannot be undone and will remove all associated data including attendance records and payments.`,
        [
  ...
  const getHeaderIcon = () => {
    return isEditMode ? 'person-circle' : 'person-add';
  };

  const getHeaderTitle = () => {
    return isEditMode ? `Edit ${currentHouseHelp?.name || 'House Help'}` : 'Add New House Help';
  };

  const getHeaderSubtitle = () => {
    return isEditMode
      ? 'Update the information for your house helper'
            ...
            <ThemedView style={styles.summaryRow}>
              <ThemedView style={styles.summaryItem}>
                <Icon
                  ...
                  color={theme.colors.primary}
                />
                <ThemedText style={[styles.summaryLabel, { color: theme.colors.text + '70' }]}>
                  Daily Wage
                </ThemedText>
                <ThemedText style={[styles.summaryValue, { color: theme.colors.text }]}>
                  {formatCurrency(currentHouseHelp.dailyWage, settings)}
                </ThemedText>
              </ThemedView>
              <ThemedView style={styles.summaryItem}>
                <Icon
                ...
                <ThemedText style={[styles.summaryValue, { color: '#FF6B6B' }]}>
                  {formatCurrency(currentHouseHelp.advancePayment || 0, settings)}
                </ThemedText>
              </ThemedView>
            </ThemedView>
            <ThemedView style={styles.summaryRow}>
              <ThemedView style={styles.summaryItem}>
                ...
                <ThemedText style={[styles.summaryValue, { color: '#4ECDC4' }]}>
                  {formatCurrency(currentHouseHelp.adjustments || 0, settings)}
                </ThemedText>
              </ThemedView>
            </ThemedView>
          </ThemedView>
      ...
      <ThemedView style={styles.buttonRow}>
        ...
        <Button
          title={isLoading
            ...
            : (isEditMode ? 'Update' : 'Add House Help')
          }
          onPress={handleSave}
          disabled={isLoading}
          loading={isLoading}
          icon={
            !isLoading && (
              <Icon
                name={isEditMode ? "save-outline" : "checkmark-outline"}
                type="ionicon"
                size={18}
                color="#FFFFFF"
                style={{ marginRight: spacing.sm }}
